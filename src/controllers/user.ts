...
export const postLogin = (req, res, next) => {
  ...
  if (validationErrors.length) {
    return req.status(404).send({ err: validationErrors });
  }
  ...
  passport.authenticate('local', (err, user, info) => {
    ...
    if (!user) {
      return req.status(404).send({ msg: info });
    }
    req.logIn(user, (err) => {
      if (err) { return next(err); }
      res.status(200).send({
        msg: 'Success! You are logged in.',
        session: req.session.returnTo,
      });
    });
  })(req, res, next);
};
...
export const logout = (req, res) => {
  ...
  req.session.destroy((err) => {
    if (err) {
      res.status(404).send({
        msg: 'Failed to destroy the session during logout.',
        err,
      });
    }
    req.user = null;
...
export const postUpdateProfile = (req, res, next) => {
  ...
  if (validationErrors.length) {
    return req.send('errors', validationErrors);
  }
  req.body.email = validator.normalizeEmail(req.body.email, { gmail_remove_dots: false });
